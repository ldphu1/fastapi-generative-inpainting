...
class InpaintingApp:
    ...
    def generate_auto_prompt(self, image_pil, mask):
        ...
        input_data = self.caption_processor(cropped_image, return_tensors="pt").to(self.device, torch.float16)
        out = self.caption_model.generate(**input_data)
    ...
    def remove_object(self, image_np, mask_np):
    ...
    def replace_object(self, image_np, mask_np, prompt):
        if image_np is None or np.max(mask_np) == 0:
            raise ValueError("Invalid image or mask")

        if not prompt or prompt.strip() == "":
            auto_label = self.generate_auto_prompt(Image.fromarray(image_np), mask_np)
        ...
        mask_lama, mask_sdxl, mask_blend = self.refine_mask(mask_np)
        ...
        final_img = self.hybrid_blend(lama_img, sdxl_img, mask_np, mask_blend)
